...
namespace ArchaeaMod.Projectiles
{
    public class magno_orb : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (Main.mouseLeft && Vector2.Distance(Main.MouseWorld - projectile.position, Vector2.Zero) < 128)
            {
                ...
                if (!mouseBox.Intersects(projectile.Hitbox))
                    projectile.position += Distance(null, Angle, 8f);
        ...
        public override void Kill(int timeLeft)
        {
            ...
            {
                int Proj1 = Projectile.NewProjectile(projectile.position + new Vector2(projectile.width / 2, projectile.height / 2), Distance(null, k, 16f), mod.ProjectileType("dust_diffusion"), 5, 4f, projectile.owner, Distance(null, k, 16f).X, Distance(null, k, 16f).Y);
